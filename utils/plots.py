...
def plot_skeleton_kpts(im, kpts, steps, orig_shape=None):
    ...
    #Líneas a trazar del skeleton
    skeleton = [[16, 14], [14, 12], [17, 15], [15, 13], [12, 13], [6, 12],
                [7, 13], [6, 7], [6, 8], [7, 9], [8, 10], [9, 11], [2, 3],
                [1, 2], [1, 3], [2, 4], [3, 5], [4, 6], [5, 7]]
    
    pose_limb_color = palette[[9, 9, 9, 9, 7, 7, 7, 0, 0, 0, 0, 0, 16, 16, 16, 16, 16, 16, 16]] #Naranja, Rosado, Azul, Verde [Líneas a trazar]
    #pose_limb_color = palette[[7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7]] #Azul, Rosado, Naranja, Verde [Líneas a trazar]
    pose_kpt_color = palette[[16, 16, 16, 16, 16, 0, 0, 0, 0, 0, 0, 9, 9, 9, 9, 9, 9]] #Verde, Naranja y Azul [0 a 16 puntos a marcar] 
    #Verde cabeza, Naranja hombros y brazos y Azul cadera y piernas
    radius = 5
    num_kpts = len(kpts) // steps #17*3/3

    for kid in range(num_kpts):
        r, g, b = pose_kpt_color[kid] #Color [0] a b [1] a g y [2] a r "al revés"
        x_coord, y_coord = kpts[steps * kid], kpts[steps * kid + 1] #Steps = 3 (desde la función principal), x =step 3x0, 3x1 , 3x2, y= step 3x0 + 1, 3x1 +1 
        if not (x_coord % 640 == 0 or y_coord % 640 == 0):
            if steps == 3:
                conf = kpts[steps * kid + 2] #el que faltaba
                if conf < 0.5: #Si es menor a este resultado 
                    continue  #No imprimimos
            cv2.circle(im, (int(x_coord), int(y_coord)), radius, (int(r), int(g), int(b)), -1) #Dibujamos los circulos

    for sk_id, sk in enumerate(skeleton): #Para trazos de los puntos
        r, g, b = pose_limb_color[sk_id] #Color [0] a b [1] a g y [2] a r "al revés"
        pos1 = (int(kpts[(sk[0]-1)*steps]), int(kpts[(sk[0]-1)*steps+1])) #Coordenadas del extremo1, el -1 es porque el inicio es [0] y no [1]
        pos2 = (int(kpts[(sk[1]-1)*steps]), int(kpts[(sk[1]-1)*steps+1])) #Coordenadas del extremo2, el -1 es porque el inicio es [0] y no [1]
        if steps == 3:
